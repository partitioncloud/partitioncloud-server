...
def can_delete_groupe(user: User, groupe: Groupe) -> None:
    if user in groupe.get_admins():
        return

    if user not in groupe.get_users():
...
@admin_bypass
@requires_login
def has_write_access_groupe(user: User, groupe: Union[Groupe, FakeObject]) -> None:
    if user in groupe.get_admins():
        return
...
def can_delete_album(user: User, album: Album) -> None:
    ...
    groupe = album.get_groupe()
    if groupe is not None:
        return has_write_access_groupe(user, groupe)
